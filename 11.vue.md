...
- v-bind: 指令 强制数据绑定 属性值是一个动态值 缩写在属性值前面添加 “ : 冒号 ”
  - demo
    ```html
...
- demo
  ```html
    <div id='app'>
        <alter>slot展示数据</alter>
        <alter></alter>  //当父组件没有填入数据的时候 自动使用默认数据填充 （在有设置默认的情况下）
    </div>
    <script>
        Vue.component('alter', {
            template: `
...
- demo
  ```html
    <div id='app'>
        ...
        <p>通过 template 模板 包裹 可以省略每条都写标识 --------------------------</p>
        <juming>
...
- setup 不能是个 async 函数 使用 async 后返回的是promise 模板会找不到return对象中的属性
- setup 函数的两种返回值
