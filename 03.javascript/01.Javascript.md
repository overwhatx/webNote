...
```
- 对象都会有一个属性 __proto__ 指向构造函数的 prototype原型对象，之所以我们对象可以使用构造函数 prototyp 原型对象的属性和方法，就是因为对象有 __proto__ 原型的存在
...
```js
  ...
  console.log(b); // eGJnMTIzNDY1eGJn
  console.log(window.atob(b)); // xbg123465xbg
```

## 闭包是什么？闭包就是函数
- 闭包 指有权访问另一个函数作用域中变量的函数 （简单理解： 一个作用域可以访问另一个函数内部的局部变量）
  ```js
  function Father() {
    let num = 123;
    function Son() {
...
```js
 <body>
  ...
  <script>
    ...
    let lis = document.querySelectorAll("li");
    for (var i = 0; i < lis.length; i++) {
      // 立即执行函数也成为闭包因为立即执行函数里面的任何一个函数都可以使用他的i变量
      (function (i) {
        lis[i].onclick = function () {
          console.log(i);
        };
...
## 递归
- 什么是递归：如果一个函数在内部可以调用其本身，那么这个函数就是递归函数 （函数内部自己调用自己，这个函数就是递归函数）
- 递归函数的作用和循环效果一样 
- 由于递归很容易发生 **栈溢出** 错误（stack overflow），所以必须要加退出条件 return
  ```js
    ...
    fn();
    //报错信息 `Maximum call stack size exceeded (超过了最大调用堆栈大小)`

    let num = 1;
    function fn() {
      console.log("只让递归循环三次");
      if (num == 3) return;
      num++;
      fn();
    }
...
- 利用递归求 n到n 的阶乘
  ...
  function fn2(min, max) {
    ...
    return max * fn2(min, max - 1);
  }
  console.log(fn2(2, 5));
  ```

## 深拷贝 浅拷贝
- 浅拷贝 浅拷贝只是拷贝一层，更深层次对象级别的只拷贝引用地址
  - 使用 for in 进行浅拷贝
  ```js
    let obj = { id: 1, name: "张三",msg: {age: 18,}};
    let o = {};
    for (let k in obj) {
      // k 为属性名 obj[k]为属性值
      o[k] = obj[k];
    }
    console.log(o); //{id: 1, name: '张三', msg: {age:19}}
    // 将拷贝后的深层次数据进行修改 被拷贝的原数据也会跟随改变
    o.msg.age = 19;
    console.log(o); //{id: 1, name: '张三', msg: {age:19}}
    console.log(obj); //{id: 1, name: '张三', msg: {age:19}}
  ```
  - 使用 Object.assign(target, ...sources) 进行浅拷贝  属性从一个或多个源对象复制到目标对象，返回修改后的对象
    - target 目标  sources 原对象
  ```js
    let a = { id: 1, name: "张三",msg: {age: 18,}};
    let b = { city: "芝加昌"};
    let o = {};
    Object.assign(o, a, b);
    o.msg.age = 19;
    console.log(a); //{id: 1, name: '张三', msg: {age:19}}
  ```
- 深拷贝 深拷贝拷贝多层，每一级别的数据都会拷贝
  - 通过 for in 以及递归 进行深拷贝
  ```js
  let obj = { id: 1, name: "张三",msg: {age: 18,},hobby: ["唱", "跳"]};
  let o = {};
  function Copy(newObj, oldObj) {
    for (let k in obj) {
      ...
      // 判断属性值的类型
      // 数据为复杂数据类型进行 递归 拷贝
      // Array 的判断要写在 Object 前面 因为 Array 也会等于Object (`let c = []; console.log(c instanceof Object); //true`)
...
## webSocket
- WebSocket 是什么？
  - WebSocket 是一种网络通信协议，设计用于提供低延迟、全双工（一种通信方式）和长期运行的连接 (是 HTML5 开始提供的一种在单个 TCP 连接上进行全双工通讯的协议)
    - 全双工：通信的两个参与方可以同时发送和接收数据，不需要等待对方的响应或传输完成
  - WebSocket目前支持两种统一资源标志符ws和wss，类似于HTTP和HTTPS

### 传统通信与实时通信的对比
- 传统通信：电子邮件、网页浏览等存在延迟，需要用户主动进行一定操作后发送请求获取更新数据
- 实时通信：即时消息传递、音视频通话、在线会议和实时数据传递等，可以实现即时的数据传输和交流，不需要用户主动请求或刷新来获取更新数据

### 没有WebSocket 前如何实时通信
- 轮询：客户端定期向服务器发送请求
- 长轮询：在客户端发出请求后，保持连接打开，等待新数据响应后再关闭连接
  - 举例：张三今天一定要取到快递，他就一直站在快递点，等待快递一到，立马取走
- Comet：保持长连接，在返回请求后继续保持连接打开

### WebSocket 带来了什么？
- 解决实时通信问题
- 双向实时通信：允许在单个、长时间的连接上进行双向实时通信，在需要快速实时更新的应用程序里，比http更加高效
- 降低延迟：链接一旦建立会保持开放，数据在客户端和服务器之间进行传输的延迟比http更低
- 更高效的资源利用：可以减少重复请求和响应开销，因为它的连接只需要建立一次

### WebSocket 心跳机制
- 为了保持webSocket 稳定的长连接，在连接建立之后，服务器和客户端之间通过心跳包来保持连接状态，以防止连接因为长时间没有数据传输而被切断
- 心跳包：一种特殊的数据包，不包含任何实际数据，仅用来维持链接状态
- 心跳包由客户端和服务器端定期发送一个空数据帧，确保链接仍然有效，避免长时间没有数据传输而被中断，如果一段时间内没有收到对方的心跳包，就可以认为连接已经断开

### WebSocket 的限制
- 不提供加密功能：如果需要安全上的需求，需要采用其他方式来确保安全性，如SSL协议，设置黑白名单
- 存在兼容性问题：不支持较老的浏览器（ie10以前的版本），需要使用 ajax 或其他方式来进行替代
### WebSocket API
- WebSocket.onclose 用于指定连接关闭后的回调函数
- WebSocket.onerror 用于指定连接失败后的回调函数
- WebSocket.onopen  用于指定连接成功后的回调函数
- WebSocket.readyState 返回当前 WebSocket 的链接状态，**只读** `var readyState = WebSocket.readyState;`
  - 0 正在链接中
  - 1 已经链接并且可以通讯
  - 2 连接正在关闭
  - 3 连接已关闭或者没有链接成功
...
- WebSocket 事件 使用 addEventListener() 或将一个事件监听器赋值给本接口的 oneventname 属性，来监听下面的事件
...
- server.js
  ...
  wss.on('request', (request, response) => {
...
- eval() 函数会将传入的字符串当做 JavaScript 代码进行执行。
  ...
  let response = "false";
    console.log(typeof eval(response), eval(response)); // boolean false
    if (eval(response)) {
      console.log("被执行");
    }
  ```
